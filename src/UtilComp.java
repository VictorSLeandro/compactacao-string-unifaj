...
public class UtilComp {
	...
	byte[] compactar(String txt, int bit){
		int t = txt.length();
		...
		byte codificado[]=new byte[(int)(tmpRet1*Math.ceil(t/tmpRet2))];
		char str[]=new char[t];
		txt.getChars(0,t,str,0);
		int chaVal = 0;
		String temp;
		String strBinario = new String("");
		for (int i = 0;i<t; i++){
			temp = Integer.toBinaryString(getValor(str[i]));
			while(temp.length()%bit != 0){
				temp="0"+temp;
			}
			strBinario=strBinario+temp;
		}
		while(strBinario.length()%8 != 0){
		   strBinario=strBinario+"0";
		}
		Integer tempInt =new Integer(0);
		for(int i=0 ; i<strBinario.length();i=i+8){
			tempInt = tempInt.valueOf(strBinario.substring(i,i+8),2);
			codificado[i/8]=tempInt.byteValue();
		}
		return codificado;
	}
	
	String descompactar(byte[] codificado, int bit){
		...
		for(int i = 0;i<codificado.length;i++){         
			if(codificado[i]<0){
				intTemp = (int)codificado[i]+256;
			}else
				intTemp = (int)codificado[i];
			strTemp = Integer.toBinaryString(intTemp);
			while(strTemp.length()%8 != 0){
				strTemp="0"+strTemp;
			}
			strBinario = strBinario+strTemp;
		}
		for(int i=0 ; i<strBinario.length();i=i+bit){
			tempInt = tempInt.valueOf(strBinario.substring(i,i+bit),2);
			strText = strText + getChar(tempInt.intValue()); 
		}
		return strText;
	}
	int getValor(char ch){
		int chaVal = 0;
		switch(ch){
